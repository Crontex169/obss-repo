...
export default function DashboardPage() {
  ...
  const [hasActivePreAssessment, setHasActivePreAssessment] = useState<boolean>()
    ...
    Promise.all(candidates.map((c) => getInterview(c.id)))
      .then((details) => {
        ...
        const withReports = candidates
          .map((c, i) => ({ createdAt: c.createdAt, report: details[i].report }))
          .filter(
            (item): item is { createdAt: string; report: Report } => item.report !== null,
          )
  ...
  return (
    <>
      <PageHeader
        title={firstName ? t('greeting', { name: firstName }) : t('greetingFallback')}
        subtitle={t('subtitle')}
        actions={
          <Link
            to="/interview/new"
        ...
        <>
          ...
          <div className="mt-8">
            <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1">
              ...
              <Link to="/interviews" className="text-sm font-medium text-[var(--color-accent)]">
                {t('recent.viewAll')}
              </Link>
            </div>
